...
words to the letters of the English alphabet acrophonically
...
(Under UNIX/Linux and Windows, something similar might exist.) Apart from the
text to be spoken, your procedure also needs to accept two additional
parameters:
...
def speakICAO(txt, icaoPause, wPause):
...
def main():
    ...
    icaoPause = float(input("Enter the length of the pause between each spoken ICAO word:\n>>"))
    wPause = float(input("Enter the length of the pause between each word spoken:\n>>"))
    speakICAO(txt, icaoPause, wPause)
